...
let server = http.createServer((req, res) => {
    ...
    if (path.extname(pathname)) { //判断有无后缀，有后缀是静态资源
        ...
        if (pathname === "/checkcode") { //判断接口是验证码
            ...
            let n1 = Math.floor(Math.random() * str.length)
            let n2 = Math.floor(Math.random() * str.length)
            let n3 = Math.floor(Math.random() * str.length)
            let chcode = str[n] + str[n1] + str[n2] + str[n3]; //四位随机验证码
